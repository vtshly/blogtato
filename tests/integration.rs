...
#[test]
fn test_add_script_creates_feed() {
    let ctx = TestContext::new();
    ctx.run(&[
        "feed",
        "add-script",
    ...
    let feeds = ctx.read_feeds();
    assert_eq!(feeds.len(), 1);
    assert_eq!(feeds[0]["url"].as_str().unwrap(), "script:hn-updates");
    assert_eq!(
        feeds[0]["command"],
        serde_json::json!(["/tmp/hn-updates.py"])
    );
}

#[test]
fn test_add_script_stores_command_with_args() {
    let ctx = TestContext::new();
    ctx.run(&[
        "feed",
        "add-script",
    ...
    let feeds = ctx.read_feeds();
    assert_eq!(feeds.len(), 1);
    assert_eq!(feeds[0]["url"].as_str().unwrap(), "script:tg-channel-2");
    assert_eq!(
        feeds[0]["command"],
        serde_json::json!([
            "python",
...
#[test]
fn test_add_script_then_sync() {
    let ctx = TestContext::new();
    let script_path = ctx.dir.path().join("hn-updates.sh");
    let script = format!(
        r#"cat <<'XML'
<?xml version="1.0" encoding="UTF-8"?>
<rss version="2.0">
  <channel>
    <title>HN updates</title>
    <link>https://hnrss.org/</link>
    <description>Generated by local script</description>
    <item>
      <title>Script Post</title>
      <pubDate>{}</pubDate>
      <guid>script-post-1</guid>
      <link>https://example.com/script-post</link>
    </item>
  </channel>
</rss>
XML
"#,
    ...
    ctx.run(&["feed", "add-script", "hn-updates", "--", "sh", script_arg])
        .success();
    ctx.run(&["sync"]).success();

    let posts = ctx.read_posts();
    assert_eq!(posts.len(), 1);
    assert_eq!(posts[0]["title"].as_str().unwrap(), "Script Post");
    assert_eq!(
        posts[0]["link"].as_str().unwrap(),
        "https://example.com/script-post"
    );

    let feeds = ctx.read_feeds();
    assert_eq!(feeds.len(), 1);
    assert_eq!(feeds[0]["url"].as_str().unwrap(), "script:hn-updates");
    assert_eq!(feeds[0]["title"].as_str().unwrap(), "HN updates");
    assert_eq!(feeds[0]["site_url"].as_str().unwrap(), "https://hnrss.org/");
    assert_eq!(
        feeds[0]["description"].as_str().unwrap(),
        "Generated by local script"
    );
    assert_eq!(feeds[0]["is_fetched"].as_bool().unwrap(), true);
    assert_eq!(feeds[0]["command"], serde_json::json!(["sh", script_arg]));
}

#[test]
fn test_add_script_then_sync_passes_args_to_script() {
    let ctx = TestContext::new();
    let script_path = ctx.dir.path().join("tg2rss.sh");
    let script = format!(
        r#"channel_url="$1"
limit="$2"
cat <<XML
...
<rss version="2.0">
  <channel>
    <title>TG ${{channel_url}}</title>
    <link>${{channel_url}}</link>
    <description>Generated with limit ${{limit}}</description>
    <item>
      <title>Telegram Post</title>
      <pubDate>{}</pubDate>
      <guid>${{channel_url}}#1</guid>
      <link>${{channel_url}}/1</link>
    </item>
  </channel>
</rss>
XML
"#,
    ...
    let posts = ctx.read_posts();
    assert_eq!(posts.len(), 1);
    assert_eq!(posts[0]["title"].as_str().unwrap(), "Telegram Post");
    assert_eq!(
        posts[0]["link"].as_str().unwrap(),
        "https://t.me/s/channel_name/1"
    );
    ...
    assert_eq!(feeds[0]["url"].as_str().unwrap(), "script:tg-channel-2");
    assert_eq!(
        feeds[0]["title"].as_str().unwrap(),
        "TG https://t.me/s/channel_name"
    );
    assert_eq!(
        feeds[0]["site_url"].as_str().unwrap(),
        "https://t.me/s/channel_name"
    );
    assert_eq!(
        feeds[0]["description"].as_str().unwrap(),
        "Generated with limit 20"
    );
    assert_eq!(
        feeds[0]["command"],
        serde_json::json!(["sh", script_arg, "https://t.me/s/channel_name", "20"])
    );
}
