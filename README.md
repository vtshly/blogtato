...
- Subscribe to scripts that generate RSS or Atom
...
For pages that do not publish RSS or Atom, you can subscribe to a local script
that prints RSS or Atom XML to stdout:

```bash
blog feed add-script <feed-name> -- </path/to/rss-scrapers/hn-updates.py>
```

Script commands can also receive arguments, which is useful for generic
scrapers:

```bash
blog feed add-script <some-site> -- <python> </path/to/scripts/page2rss.py> <argument-for-script like url>
```

Everything after `--` is stored as separate command arguments:

```json
{
  "url": "script:some-site",
  "command": [
    "python",
    "/path/to/scripts/page2rss.py",
    "https://site/page_name"
  ]
}
```

Script feed names are stored as `script:<name>`, so the first example stores a
subscription like this after it has been synced:

```json
{
  "id": "fa61c6ee624",
  "url": "script:hn-updates",
  "title": "HN updates",
  "site_url": "https://hnrss.org/",
  "description": "Generated by local script",
  "is_fetched": true,
  "command": ["/path/to/rss-scrapers/hn-updates.py"]
}
```

`blog sync` runs the command every time that source is synced, parses the
command's stdout as RSS or Atom, and stores the resulting posts like any other
feed. Commands are executed directly, not through a shell; add `sh`, `python`,
or another interpreter explicitly when you need one:

```bash
blog feed add-script example -- sh /path/to/scraper.sh
```

Because subscription data can be synced through git, only add script feeds from
stores you trust. A script source is local executable code.
